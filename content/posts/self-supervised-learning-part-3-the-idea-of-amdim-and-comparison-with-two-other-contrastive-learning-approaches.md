...
where ![equation](https://latex.codecogs.com/gif.image?%5Cdpi%7B110%7D%20%5Cinline%20N_%7B7%7D) is a set of negative samples and ![equation](https://latex.codecogs.com/gif.image?%5Cdpi%7B110%7D%20%5Cinline%20L_%7B%5CPhi%7D) is a softmax function:
...
Here, *f* and theta ![equation](https://latex.codecogs.com/gif.image?%5Cdpi%7B100%7D%20%5Cinline%20%5CPhi) are two parametric functions that need to be calculated. In this case, these functions are deep neural networks and parameters are their weights. Concretely, ![equation](https://latex.codecogs.com/gif.image?%5Cdpi%7B100%7D%20%5Cinline%20%5CPhi) maps a pair of (antecedent feature, consequent feature) into a single scalar value/a score. The higher score leads to the higher possibility that this is a positive pair which means both the antecedent and consequent are extracted from the same sample.

Now that we have understood the symbols, let’s analyze a little bit more about the softmax function. In the numerator is the exponential of the score of a pair of antecedent and consequent (can be positive or negative). In the denominator, ![equation](https://latex.codecogs.com/gif.image?%5Cdpi%7B100%7D%20%5Cinline%20%5Ctilde%7Bf%7D_%7B7%7D) is got from the union set of negative samples and positive samples, then the sum of exponentials is calculated. Their goal is to make this fraction high in the case of the positive pair and low in the case of the negative pair. The more this becomes true, the lower the loss function will be. What is the connection with the maximization of NCE bound? 
